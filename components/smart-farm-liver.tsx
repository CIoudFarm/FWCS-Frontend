...
export default function SmartFarmLiver({
  userConfig,
  baseConfig,
    ...
    if (mode === "random") {
      const interval = setInterval(() => {
        setSensorData((prevData) => {
          const tempChange = (Math.random() - 0.5) * 0.1; // -0.05 ~ +0.05도 변화
          const humChange = (Math.random() - 0.5) * 0.2; // -0.1 ~ +0.1% 변화
          const newTemp = +(prevData.temperature + tempChange).toFixed(2);
          const newHum = +(prevData.humidity + humChange).toFixed(2);
          return {
            temperature: newTemp,
            humidity: newHum,
          };
        });
      }, 10000); // 0.5초 간격으로 업데이트
      ...
      <div className="mb-2 text-sm flex flex-col items-center gap-2">
        ...
        {mode === "manual" && (
          <div className="flex flex-col gap-2 text-gray-800">
            <label>
              현재 온도: {manualSensorData.temperature}°C
              <input
                type="range"
                min="10"
                max="40"
        ...
        <div className="mb-6">
          <div className="flex items-center gap-6 font-size-100">
            <div className="flex items-center">
              <Thermometer className="h-5 w-5 mr-1 text-red-500" />
              <h1 className="text-lg">현재 온도: {sensorData.temperature}°C</h1>
            </div>
            <div className="flex items-center">
              <Droplet className="h-5 w-5 mr-1 text-blue-500" />
              <span className="text-lg">현재 습도: {sensorData.humidity}%</span>
            </div>
          </div>
        </div>
